...
class DemoScriptFactory:
    def __init__(self, script: ScriptFile, paths: MaterialPaths):
        self.script = script
        self.paths = paths
    ...
    def add_loop_video_to_track(
        ...
        video_duration = self.get_media_duration(video_path, "Video")
        current = start_time
    ...
    def add_subtitle(self, subtitle_file: str, current_start: int, text_track_name: str):
        self.script.add_track(TrackType.text, text_track_name, relative_index=250)
        suffix = Path(subtitle_file).suffix.lower()
        subtitle_path = subtitle_file

        if suffix == ".lrc":
            os.makedirs(self.paths.srt_output_dir, exist_ok=True)
            subtitle_path = os.path.join(self.paths.srt_output_dir, f"{text_track_name}.srt")
            lrc_to_srt(subtitle_file, subtitle_path)
        elif suffix != ".srt":
            raise ValueError(f"不支持的字幕格式: {suffix}")

        style_reference = draft.TextSegment(
            self.short_name(os.path.basename(subtitle_file)),
    ...
    def add_video_grid(self, video_items: list[MusicItem], start_time: float, duration: float, track_prefix: str, width: int, height: int):
        if not video_items:
            return
        ...
        for i, item in enumerate(video_items):
            ...
            selected_video = os.path.join(self.paths.video_dir, random.choice(os.listdir(self.paths.video_dir)))
            ...
            self.script.add_track(draft.TrackType.video, track_name, relative_index=i)
            self.script.add_segment(video_segment, track_name)

            text_segment = TextSegment(
                self.short_name(item.name),
            ...
            self.script.add_track(draft.TrackType.text, text_track_name, relative_index=0)
            self.script.add_segment(text_segment, text_track_name)

    def main_audio_with_video(self, audios: list[str], subtitles: list[str], current_start: int, sum_video: list[MusicItem]):
        ...
        for track_type, track_name, index in [
            ...
            if index is None:
                ...
                self.script.add_track(track_type, track_name, relative_index=index)

        videos = set()
        while len(videos) < len(audios):
            videos.add(random.choice(os.listdir(self.paths.video_dir)))

        sum_time = current_start
        for i, audio in enumerate(audios):
            audio_file = os.path.join(self.paths.audio_dir, audio)
            duration_sec = self.get_media_duration(audio_file, "Audio")
            ...
            if videos:
                ...
                output_path = apply_video_mask(
                    os.path.join(self.paths.video_dir, current_video),
                    os.path.join(self.paths.mask_dir, random.choice(os.listdir(self.paths.mask_dir))),
                    os.path.join(self.paths.intro_temp_dir, f"{uuid.uuid4().hex}.mov"),
                ...
                self.script.add_segment(mask_segment, linked_track_name)
                self.add_loop_video_to_track(
                    os.path.join(self.paths.video_dir, next_video),
                    video_track_name,
            ...
            self.add_loop_video_to_track(
                ...
                video_track_name,
                current_start,
                duration_sec - linked_video_duration,
            )
            ...
            self.script.add_segment(visual_segment, visual_track_name)

            self.add_subtitle(subtitles[i], current_start=current_start, text_track_name=f"{subtitle_track_prefix}{i}")
            audio_segment = draft.AudioSegment(audio_file, trange(f"{current_start}s", f"{duration_sec}s"), volume=0.6)
            self.script.add_segment(audio_segment, audio_track_name)

            current_start += duration_sec
            if videos:
                self.add_video_grid(sum_video, start_time=current_start + 2, duration=4, track_prefix="九宫格", width=1920, height=1080)
...
# ===== 兼容旧调用的函数 API =====
def short_name(name: str, max_len: int = 9) -> str:
    return DemoScriptFactory.short_name(name, max_len)


def get_media_duration(path: str, track_type: str) -> float:
    return DemoScriptFactory.get_media_duration(path, track_type)


def add_loop_video_to_track(script: ScriptFile, video_path: str, track_name: str, start_time: float, duration: float, intro_type=None, change_type=None):
    factory = DemoScriptFactory(script=script, paths=MaterialPaths(root_dir="", draft_root=""))
    return factory.add_loop_video_to_track(video_path, track_name, start_time, duration, intro_type, change_type)


def add_subtitle(script: ScriptFile, paths: MaterialPaths, subtitle_file: str, current_start: int, text_track_name: str):
    factory = DemoScriptFactory(script=script, paths=paths)
    return factory.add_subtitle(subtitle_file, current_start, text_track_name)


def add_video_grid(script: ScriptFile, paths: MaterialPaths, video_items: list[MusicItem], start_time: float, duration: float, track_prefix: str, width: int, height: int):
    factory = DemoScriptFactory(script=script, paths=paths)
    return factory.add_video_grid(video_items, start_time, duration, track_prefix, width, height)
...
def main_audio_with_video(script: ScriptFile, paths: MaterialPaths, audios: list[str], subtitles: list[str], current_start: int, sum_video: list[MusicItem]):
    factory = DemoScriptFactory(script=script, paths=paths)
    return factory.main_audio_with_video(audios, subtitles, current_start, sum_video)
...
def run_demo(material_root: str = r"D:\video", draft_root: str = r"D:\jianying\JianyingPro Drafts"):
    ...
    factory.add_video_grid(video_list, start_time=current_time, duration=4.5, track_prefix="视频轨", width=1920, height=1080)

    audios = sorted(os.listdir(paths.audio_dir))
    subtitles = [os.path.join(paths.subtitle_dir, "test.lrc") for _ in audios]
    current_time += factory.main_audio_with_video(audios, subtitles, current_start=3, sum_video=video_list)
    script.save()
